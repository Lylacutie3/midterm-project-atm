...
private:
    ...
    Account acc;
public:
    string findUSBDrive() {//this function look for removable drive like usb
        char drive = 'A';
        char driveName[] = "A:\\";

        for (drive = 'A'; drive <= 'Z'; ++drive) { //[1]loop all letters to what letter is the removable drive
            driveName[0] = drive; 
            UINT driveType = GetDriveType(driveName);
            if (driveType == DRIVE_REMOVABLE) {
                return string (driveName);}
        }
        return ""; // [2] return nothing if there is no removable file found
    }

    bool AccInitialize(){ //this function will now look for a specific filee
        usbDirectory = findUSBDrive();//[3] in here it will pass first the letter of the removable drive found
        if (usbDirectory.empty()){ 
            return false; //[4]look for a file insede a drive, if there no file found it will return false 
        }
        filePathUSB = usbDirectory + "\\USBAccount.txt"; 
        return true;//[5] if the file found name ^ it will return true that file exist
    }
    ...
    ofstream file(filePathUSB, ios::out); //[]write if the file exist when not it will create and write
    if (!file) { 
    ...
    void manuallyEject(){ // this function is for manually eject
        cout << "Please manually eject your USB"<<endl; //[]promt to eject usb 
        while(AccInitialize()){ // [] this will wait until there is no file found 
            cout << "."; //use AccInitialize cause i where by this time we are inside the removable drive
        }
...
public:
    ...
    void add(Account s){ // this function is use to add a new account
        LinkList* newAcc = new LinkList(s); //[]create a new Node to be added
        LinkList* q, *p;
        if(head == NULL){ //[]add into head if the head is empty
            head = newAcc;
            return;
        }
        q=p=head;//[]initialized the p and q pointer as head to use in loop
        while(p != NULL && p->info.lastname < s.lastname){
            q = p;
            p = p->next;
        }
        if(p->info.lastname > s.lastname){ //ths use to make the record alpahabetical
            newAcc->next = p;//[] make the next of new acc as p
            q->next = newAcc;//and make q next as newAcc to put it between
        }else{
            q->next = newAcc;//[]it will be add to last if its not lower that other names
        }
    }
    ...
    void deposit(LinkList* p){
        cout << "Warning! No coins only paper bill"<<endl; //[] warning for user
        cout << "Input how much deposit: "; 
    ...
    void transfer(LinkList* p){// this function is for transfer fund
        ...
        cout << "You only can transfer to enrolled Account" << endl;
     ...
     void changePin(LinkList* p){ //inserted (datavalidation, censoredship)
        ...
        char ch1, ch2, ch3;
        Account acc; acc.accountNo = p->info.accountNo;
...
string getnumber() {
    ...
    return "+639" + to_string(v.validationOthers(2));
}
...
public:
    ...
    void withdraw(Account s){ // withdraw with Info s to know the blance of wothdrawer
        ...
        if (withdraw%100 == 0){ //[]check if its multiple by a hundred
            if(p->info.balance>=withdraw){ //[]check if balance is enough
                ...
                Sleep(1000);
                balanceInquiry(s); //[]prompt the balance after withdraw
                return;
            }else{
                cout << "\nInsufficient Balance!!!" << endl;
                cout << "\nWithdraw unsaccessful!" <<endl;
                Sleep(1000);
                balanceInquiry(s); //[]prompt the balance available
                Sleep(1000);
                transac(s); //[]call transact again
            }
        }else{
    ...
    void anotherTransaction(Account s){ //ask for another transaction
        ...
        cin >> pick; //[] ask user for another transaction
        switch (pick) {
            case 1: transac(s);break; //[]go to menu if want another
            case 2: break; //[]home again //exit loop of Transac
        }
    }
...
int main(){
    ...
    return 0;
        ...
        if (p == NULL) {
            ...
            cin.get();
            r.reg();  // Call registration process
        } else {
